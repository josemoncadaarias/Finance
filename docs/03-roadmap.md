...
## Phase 2 — Seeding the database (done, and then removed)

The five years of history Jose already had were read in once, cleaned up and
reconciled. The script that did it, its review queue and the exports it read
were deleted on 2026-09-12 and 2026-09-22: every movement is entered by hand
now, and nothing about the data as it stands is decided by reading an old
export of it.

## Phase 3 — Base UI

**Deliverable:** an app usable day to day. At this point it replaces the old app,
which means matching what the old app is actually good at, not merely showing the
same data.

Specified by Jose on 2026-09-09, from using the old app daily. The order below is
the order to build in: the filter state comes first because everything else
reads from it.

### 3.1 One filter, shared by the whole screen

Two controls that every view obeys at once, the way the old app does it:
...
That gesture is most of why the old app feels quick.
...
The donut the old app opens on: each category as a slice with its share of the
...
The old app puts three figures in the middle of it. Two are useful — money spent and
...
each: **green in, red out, neutral moved**. The old app paints a transfer red like
any expense, which is exactly the confusion worth not inheriting.

### 3.4 Entry and editing

- Fast entry, few taps. This is the old app's real strength and the thing most worth
  copying carefully.
...
The old app keeps its settings in a drawer. Going through them one by one, against
what this app is for:

| The old app's setting | Here |
|---|---|
| **Traslado** (carry the previous period's balance forward) | **Not needed, and already answered better.** It exists because the old app's headline figure is the period's arithmetic, so without it a month starts at zero and the balance means nothing. This app shows the account's real balance today, above the period, so there is nothing to carry. |
...
| **Copia de seguridad / restaurar / borrar datos** | **Yes, and more important than in the old app**, because this database is the only copy. A single file that can be copied off the phone and read back. Belongs beside export. |
...
Two settings this app needs that the old app has no reason to:
