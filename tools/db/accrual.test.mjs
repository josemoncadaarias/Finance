...
test('a product can hand all its movements to another', async () => {
  ...
  // one", so making a product usual moved years of movements into it.
  const { db, accounts, transactions, yields, engine, ids } = await setup();
  ...
  const [savings] = await yields.pockets(plata);
  await db.run("UPDATE yield_pockets SET source = 'manual' WHERE id = ?", [savings.id]);
  await yields.setPocketBalance({ pocket_id: savings.id, valid_from: '2026-09-01', amount_minor: 0 });
  const bolsillo = await yields.addPocket({
    account_id: plata, name: 'Bolsillo', source: 'manual', sort_order: 1,
  });
  await yields.setPocketBalance({ pocket_id: bolsillo, valid_from: '2026-09-01', amount_minor: 0 });

  // Each product earns at its own rate, as Plata's two do.
  await yields.setRate({
    account_id: plata, pocket_id: savings.id, component: 'base', payout: 'daily',
    valid_from: '2026-09-01', annual_rate_scaled: pct(6.5),
  });
  await yields.setRate({
    account_id: plata, pocket_id: bolsillo, component: 'base', payout: 'daily',
    valid_from: '2026-09-01', annual_rate_scaled: pct(11),
  });

  // The money lands on the savings product, which is where the old reading
  // put it - and where it never was.
  await transactions.create({
    account_id: plata, category_id: ids.gastos, pocket_id: savings.id,
    occurred_on: '2026-09-02', amount_minor: 200_000_00,
  });
  await engine.accrue(plata, '2026-09-08');

  const wrong = await yields.days(plata);
  assert.ok(wrong.some(day => day.pocket_id === savings.id && day.balance_minor === 200_000_00),
    'the savings product was earning on it');

  // Corrected: all of its movements belong to the other product.
  const moved = await yields.movePocketMovements(savings.id, bolsillo);
  assert.equal(moved, 1);

  await yields.clearDays(plata);
  await engine.accrue(plata, '2026-09-08');

  const right = await yields.days(plata);
  assert.deepEqual(
    right.filter(day => day.pocket_id === savings.id).map(day => day.balance_minor),
    right.filter(day => day.pocket_id === savings.id).map(() => 0),
    'the product that never held it earns nothing');
  assert.ok(right.some(day => day.pocket_id === bolsillo && day.balance_minor === 200_000_00),
    'and the one that did earns on all of it');
});
